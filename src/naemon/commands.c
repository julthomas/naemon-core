...
static int command_input_handler(int sd, int events, void *discard)
{
	...
	while ((buf = iocache_use_delim(command_worker.ioc, "\n", 1, &size))) {
		...
		if ((cmd_ret = process_external_command1(buf, 1)) != CMD_ERROR_OK) {
...
struct external_command /*@null@*/ * command_parse(const char * cmdstr, int mode, int *error)
{
	...
	*error = CMD_ERROR_OK;
	...
	if (ext_command == NULL) {
		...
		if (*error != CMD_ERROR_OK && *error != CMD_ERROR_CUSTOM_COMMAND) {
...
int process_external_commands_from_file(char *fname, int delete_file)
{
	...
	while (1) {
		...
		/* process the command */
		process_external_command1(input, 1);
	}
...
int process_external_command1(char *cmd, int use_old_style_interface)
{
	...
	if (use_old_style_interface)
		mode |= COMMAND_INTERFACE_OLD_STYLE;
